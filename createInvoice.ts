...
export function createInvoice(invoice : penerimaanGetah[], path : string) {
...
function generateSubtotal(rows : penerimaanGetah[]){
  let subTotal = 0;
  rows.map((row)=>{
...
function generateCustomerInformation(doc : any, invoice : penerimaanGetah[]) {
  doc
    .fillColor("#444444")
    .fontSize(20)
  ...
  doc
    ...
    .text(
      formatCurrency(parseInt(invoice[0].total)),
      150,
...
function generateInvoiceTable(doc : any, invoice : penerimaanGetah[]) {
  let i;
